...
class Runner(EncodingHelper):
	...
	def on_signal(cls, signum, stackframe):
		"""
		All runners "pause" on interrupt, handling only urgent() input 
		to prevent any output from being printed.
		"""
		if signum == 2:
			cls.__interrupted += 1
	...
	def __init__(self, config=None, **k):
		...
		self.__write = dw = config.get('write', sys.stdout)
		self.__newl = config.get('newl', '\r\n')
		
		# choose the correct writer for the pause-status
		self.__writer = Buffer(*self.ek) if self.paused() else dw
