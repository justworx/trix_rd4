...
class IRCConnect(Connect):
	...
	def __init__(self, config=None, **k):
		...
		self.__show      = config.get('show', True) # print lines
	...
	def __plugin_unload(self, pname):
		...
		try:
			...
			trix.log ("__plugin_unload", pname)
			raise
	...
	def __handle_plugin_rmv(self):
		try:
			...
			trix.log ("irc_connect.__handle_plugin_rmv", 
					str(type(ex), str(ex))
				)
		finally:
			self.pm_rmv = []
	...
	def __handle_plugin_add(self):
		...
		try:
			...
			trix.log ("irc_connect.__handle_plugin_add", 
